...
public class indexController {
    ...
    @GetMapping(value = "/model_date")
    @ResponseBody
    public DataStreamEntity modelShuju(Integer id) {
        System.out.println("model_date eid: " + id);
    ...
    @GetMapping(value = "/iotcloud_news")
    public String iotcloudNews(Model model, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user != null) {
            int uid = user.getId();
            List<Product> productlist = productMapper.selectByUid(uid);
